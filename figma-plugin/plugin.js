...
function createFrame({ name = "Frame", width = 800, height = 600, x = 0, y = 0, parentId }) {
  ...
  const parent = parentId ? getNode(parentId) : page();
...
async function addText({ text, x = 0, y = 0, fontFamily = "Inter", fontStyle = "Regular", fontSize = 32, hex, parentId }) {
...
function layoutGridAdd({ nodeId, pattern = "COLUMNS", count = 12, gutterSize = 20, sectionSize = 80, hex = "#E5E7EB", opacity = 0.5 }) {
  ...
  grids.push({ pattern, count, gutterSize, sectionSize, visible: true, color: { r: rgb.r, g: rgb.g, b: rgb.b, a: opacity } });
  n.layoutGrids = grids;
  return nodeInfo(n);
}
...
function createComponent({ name = "Component", fromNodeIds }) {
  ...
  if (Array.isArray(fromNodeIds) && fromNodeIds.length) {
    for (const n of fromNodeIds.map(getNode)) c.appendChild(n);
  }
  return nodeInfo(c);
}
